...
public class Controller : MonoBehaviour
{
    ...
    public struct AIBoard
    {
        ...
        public List<GamePiece> white;
        public List<GamePiece> black;
        public void GetBoardValue(int colour)
        {
            totalBoardValue = 0;
            if (colour == 0)
            {
                foreach (GamePiece piece in white)
                {
                    totalBoardValue += piece.basePower;
                }
                foreach (GamePiece piece in black)
                {
                    totalBoardValue -= piece.basePower;
                }
            }
    ...
    public void InitialBoardState(GameObject piece)
    {
        UnitInfo info = piece.GetComponent<UnitInfo>();
        GamePiece newPiece = new GamePiece();
        newPiece.basePower = info.basePower;
        newPiece.colour = info.colour;
        newPiece.moved = false;
        newPiece.returnPosition = info.returnPosition;
        newPiece.targetPosition = info.targetPosition;
        newPiece.type = info.type;
        if(piece.transform.position.x == 0)
        {
            newPiece.xPos = 0;
        } else
        {
            newPiece.xPos = (int)piece.transform.position.x/2;
        }
        if(piece.transform.position.z == 0)
        {
            newPiece.yPos = 0;
        } else
        {
            newPiece.yPos = (int)piece.transform.position.z / 2;
        }
        if(newPiece.colour == 0)
        {
            aiGameBoard.white.Add(newPiece);
        } else
        {
            aiGameBoard.black.Add(newPiece);
        }
    }
        ...
        for (int x = 0; x < boardLength; x++)
        {
            for (int y = 0; y < boardLength; y++)
            {
                ...
                if (Physics.Raycast(ray, out hit))
                {
                    ...
                    {
                        tiles[x, y].piece = hit.collider.gameObject;
                        tiles[x, y].piece.GetComponent<UnitInfo>().returnPosition = targetPosition;
                    }
                }
            }
        }
    ...
    void Move(GameObject piece,Tile startTile, int distance, int xDir, int yDir)
    {
        for(int i = 1; i <= distance; i++)
        {
            ...
            if (xOffset < boardLength && xOffset >= 0)
            {
                ...
                {
                    if(tiles[xOffset, yOffset].piece != null)
                    {
                        if(tiles[xOffset, yOffset].piece.GetComponent<UnitInfo>().colour != currentTurn)
                        {
    ...
    void IndividualKnightMove(GameObject piece,Tile startTile,int x, int y)
    {
        ...
        if (yPos < boardLength && yPos >= 0)
        {
            ...
            {
                if (tiles[xPos, yPos].piece == null)
                {
                    NewMove(xPos, yPos, piece,gameBoard);
                }
                else if (tiles[xPos, yPos].piece.GetComponent<UnitInfo>().colour != currentTurn)
                {
                    NewMove(xPos, yPos, piece,gameBoard);
                }
            }
            ...
            {
                if (tiles[xPos2, yPos].piece == null)
                {
                    NewMove(xPos2, yPos, piece,gameBoard);
                }
                else if (tiles[xPos2, yPos].piece.GetComponent<UnitInfo>().colour != currentTurn)
                {
                    NewMove(xPos2, yPos, piece,gameBoard);
                }
            }
    ...
    void PawnMove(GameObject piece,Tile startTile)
    {
        ...
        if (startTile.xPos + 1 < boardLength)
        {
            ...
            {
                if(tiles[startTile.xPos+1, startTile.yPos+dir].piece != null)
                {
                    if (tiles[startTile.xPos + 1, startTile.yPos + dir].piece.GetComponent<UnitInfo>().colour != currentTurn)
                    {
    ...
    void CheckPawn()
    {
        ...
        for(int x = 0; x < boardLength; x++)
        {
            if(tiles[x,0].piece != null)
            {
                UnitInfo info = tiles[x, 0].piece.GetComponent<UnitInfo>();
            ...
            } if (tiles[x, boardLength-1].piece != null){
                UnitInfo info = tiles[x, boardLength-1].piece.GetComponent<UnitInfo>();
